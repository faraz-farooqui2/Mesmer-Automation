...
//MR-Replay-25 | Behavior when replay test case is failed due to "Screen Loading Error"
...
def getErrorsFromTestStepDialog(){
	...
	if (WebUI.verifyElementPresent(findTestObject('Object Repository/OR_Replay/list_errorPoint'), 10)){
		...
		for (WebElement webElement : error) {
			...
			if(errorText.contains("Screen Loading Error")){
		...
		if (errorStr.contains("Screen Loading Error")){
...
def getFailuresFromTestStepDialog(){
	...
	if (WebUI.verifyElementPresent(findTestObject('Object Repository/OR_Replay/list_failedOnScreen'), 10)){
		...
		for (WebElement webElement : failed) {
			...
			if(failedText.contains("Screen Loading Error")){
		...
		if (failureStr.contains("Screen Loading Error")){
