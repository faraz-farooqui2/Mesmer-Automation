...
try{
	...
	if(WebUI.waitForElementPresent(counterTotalTC,10) && WebUI.waitForElementPresent(textTotalTC,10)){
		...
		findTestCaseAndPerformAction("ManageTest05","edit");
		// Select reDoFromHere Option
		checkAndClickTheScreenShot("reDoFromHere");
		// Select Add assertion option
		checkAndClickTheScreenShot("assertion");
		// Select Add assertion option
		checkAndClickTheScreenShot("data");
		// Select Add assertion option
		checkAndClickTheScreenShot("delete");
...
def checkAndClickTheScreenShot(String action){
	...
	if(testScreensList != null && testScreensList.size() > 1){
		...
		if(WebUI.waitForElementVisible(btnDots, 60)){
					...
					if(WebUI.waitForElementVisible(selectDeviceOption, 60)){
						...
						if(devicesList != null && devicesList.size() > 0){
							...
							if(provisionedDevicesList != null && provisionedDevicesList.size() > 0){
								...
								KeywordUtil.markWarning('WARNING: There is no provisioned device available')
							}
						}
						else{
						...
						KeywordUtil.markWarning('WARNING: Select device option not available')
					}
				}
				else{
			...
			KeywordUtil.markFailed('ERROR: Dots icon not visible')
		}
	}
	else{
		KeywordUtil.markFailed('ERROR: There is no test case screen available')
	}
}
