...
try{
	...
	if(CustomKeywords.'com.mesmer.Utility.selectProject'(projName, xpathString, platformName, ProjectName)){
		...
		if(CreateTestController.getInstance().checkIfCreateNewTestScreenOpen()){
			...
			//				String deviceNameElement = Utility.selectDeviceAndSetDeviceParam("", "",device.toString(), lang , region, coordinates, tag,deviceUDID.toString())
			List<WebElement> virtualDevicesList = Utility.getAvailableDevicesList(Device.toString())
			if(virtualDevicesList != null && virtualDevicesList.size() >=1){
...
private void waitForDeviceAndBuildInstallation(String deviceName){
	if(WebUI.waitForElementVisible(recordingStarted, 240)){
		...
		WebUI.delay(2)
		if(jsonDeviceName.equals("") || jsonDeviceName == null){
			MesmerRecordHelper.getInstance().parseJsonAndPerformActions(jsonProjectName, platformName, appUserName, appPassword)
			setTestCaseName()
		}else if(deviceName.contains(jsonDeviceName)){
...
private boolean startRecording(){
	...
	if(CreateTestController.getInstance().checkIfCreateNewTestScreenOpen()){
		...
		if(virtualDevicesList != null && virtualDevicesList.size() >=1){
			String deviceNameElement = Utility.selectDeviceAndSetParams("", "", Device.toString(), lang , region, coordinates, tag)
