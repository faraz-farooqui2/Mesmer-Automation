...
def DeviceList(){
	...
	if( Utility.getPlatformName() == "Android"){
		...
		if(readyAndroidCount > 0){
			...
			MesmerLogUtils.markWarning("No Android Ready device available")
		}
		...
		if (inUseAndroidCount > 0){
			...
			MesmerLogUtils.logInfo("No Android device in-use")
		}
		...
		if (provisionedAndroidCount > 0){
			...
			MesmerLogUtils.logInfo("No Android device Provisioned state")
		}

		//Android Broken Devices check
		if(brokenAndroidCount > 0){

			MesmerLogUtils.markWarning("Total -> "+ brokenAndroidCount + " Android devices available in Broken state")

		}else{
			MesmerLogUtils.markPassed("No Android device is in Broken state")
		}
		...
		if(readyiOSCount > 0){
			...
			MesmerLogUtils.markWarning("No iOS Ready device available")
		}
		...
		if(inUseiOSCount > 0){
			...
			MesmerLogUtils.logInfo("No iOS device in-use")
		}
		...
		if(provisionediOSCount > 0){
			...
			MesmerLogUtils.logInfo("No iOS device Provisioned state")
		}

		//iOS Broken Devices check
		if(brokeniOSCount > 0){
			MesmerLogUtils.markWarning("Total -> "+ brokeniOSCount + " iOS devices available in Broken state")

		}else{
			MesmerLogUtils.markPassed("No iOS device is in Broken state")
		}
