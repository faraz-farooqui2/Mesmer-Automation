...
//List Devices
//Test Scope : App Map devices list
	...
	if(CustomKeywords.'com.mesmer.Utility.selectProject'(projName, xpathString, platformName, ProjectName)){
		...
		if(CustomKeywords.'com.mesmer.Utility.goToAppMap'()){
			...
			if(appMapDeviceList()){
				...
				MesmerLogUtils.markFailed("App Map device list method failed")
			}
		}else{
...
private boolean appMapDeviceList(){
				...
				if(AppMapController.getInstance().clickNextButton()){
					...
					if(AppMapController.getInstance().verifyDeviceDialogue()){
						...
						if(AppMapController.getInstance().clickCloseButton()){
							...
							MesmerLogUtils.logInfo("Could not click on Close device dialogue")
						}
					}else{
...
def DeviceList(){
	...
	if( Utility.getPlatformName() == "Android"){
		...
		if (inUseAndroidCount > 0){
			...
			MesmerLogUtils.logInfo("No Android device is in Use state")
		}
		...
		if(inUseiOSCount > 0){
			...
			MesmerLogUtils.logInfo("No iOS device is in use state")
		}
