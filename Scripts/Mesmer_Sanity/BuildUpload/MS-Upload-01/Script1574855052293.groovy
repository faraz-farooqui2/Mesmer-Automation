...
try{
	WebUI.delay(5)
	TestObject firstTimeUpload = findTestObject('Object Repository/OR_BuildUpload/firstTimeUpload')
	if(WebUI.waitForElementPresent(firstTimeUpload, 10) == true){
		WebUI.click(firstTimeUpload)
									...
									if(WebUI.waitForElementVisible(nextButton, 20)==true){
										...
										if(WebUI.waitForElementVisible(projectNameField, 20)==true){
											...
											if(WebUI.waitForElementVisible(nextButton, 20)==true){
												...
												TestObject successfulBuildUpload = findTestObject('Object Repository/OR_BuildUpload/text_buildUploaded')
												if (WebUI.waitForElementVisible(successfulBuildUpload, 120)==true){
														...
														if(WebUI.waitForElementVisible(projectNameField, 20)==true){
															...
															if(WebUI.waitForElementVisible(nextButton, 20)==true){
																...
																if(WebUI.waitForElementPresent(txtProjectAlreadyExists, 10)){
																	...
																	//Check Success message on uploading project
																	TestObject successfulBuildUpload = findTestObject('Object Repository/OR_BuildUpload/text_buildUploaded')
																	if (WebUI.waitForElementPresent(successfulBuildUpload, 10)){
																		MesmerLogUtils.markPassed("Build uploaded successfully")

																		WebUI.delay(2)
																		if(WebUI.waitForElementPresent(closeWindowBtn, 10)==true){
																			WebUI.delay(2)
																			WebUI.click(closeWindowBtn)
																			MesmerLogUtils.markPassed("Build uploaded windows closed")
																		}
