...
//CreateNewTestCase-03 - Giving name to a test case
...
try{
	...
	if(WebUI.waitForElementPresent(startRecordingIcon, 90)==true){
		...
		if(WebUI.waitForElementPresent(enabledOptionsIcon, 60)==true){
			WebUI.click(enabledOptionsIcon)
			WebUI.delay(1)
			if(WebUI.waitForElementPresent(discardOption, 60)==true){
				WebUI.click(discardOption)
				WebUI.delay(1)
				if(WebUI.waitForElementPresent(textYes, 60)==true){
					WebUI.click(textYes)
					KeywordUtil.markPassed("PASSED: CreateNewTestCase-03 successfull")
				}
				else{
					KeywordUtil.markFailed("FAILED: Yes text not found")
				}
			}
			else{
