...
try{
	...
	if(searchedReadyDevice != null){
		...
		if(CreateTestController.getInstance().deviceChecks()){
			...
			String closeApplicationXpath = findTestObject('Object Repository/OR_CreateNewTestCase/iconWrap_closeApplication').findPropertyValue('xpath').toString()
			WebElement closeApplicationHover = driver.findElement(By.xpath(closeApplicationXpath))
			if(closeApplicationHover != null){
				Actions builder = new Actions(driver);
				builder.moveToElement(closeApplicationHover).perform()
			}
		}else{
