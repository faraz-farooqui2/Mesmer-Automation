			...
			if(CreateTestController.getInstance().clickDoneGreenButton()){
				if(CreateTestController.getInstance().saveNewTestCase()){
					if(CreateTestController.getInstance().clickOnReturnToDevice()){
						if(CreateTestController.getInstance().verifyAvailableOptionsReturnToDevice()){
							if(CreateTestController.getInstance().backReturnToDevice()){
								
							}else{
							MesmerLogUtils.markFailed("Unable to click on back arrow return to device / unable to verify all the options available")
							}

						}else{
							MesmerLogUtils.markFailed("Available options are not correct when clicking on return to device")
						}

					}else{
						MesmerLogUtils.markFailed("Unable to click on return to device option")
					}
				}else{
