					...
					if(CreateTestController.getInstance().clickOnViewTest()){
						if(CreateTestController.getInstance().verifyAvailableOptionsViewTest()){
							if(CreateTestController.getInstance().viewAndEditTestCase()){
								if(WebUI.waitForElementPresent(titleTextEditText, 30)){	
								}else{
								MesmerLogUtils.markFailed("User is not on a edit test case screen")
								}
							}else{
								MesmerLogUtils.markFailed("Unable to click on view and edit test case")
							}
						}else{
