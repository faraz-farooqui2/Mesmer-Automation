...
try{
	...
	if(searchedReadyDevice != null){
		...
		if(CreateTestController.getInstance().deviceChecks()){
			...
			if(WebUI.waitForElementPresent(dropdownLogType, 20)){
				WebUI.click(dropdownLogType)
				
				WebUI.click(btnVerbose)
				...
				WebUI.click(btnError)
				
			}else{
			MesmerLogUtils.markFailed("Unable to click on dropdown log type")
			}
		}else{
