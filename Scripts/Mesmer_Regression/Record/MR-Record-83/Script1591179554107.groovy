					...
					if(CreateTestController.getInstance().verifyOptionsAvailableOnSetting()){
						...
						if(WebUI.waitForElementPresent(btnFilter, 20)){
							...
							if(WebUI.waitForElementPresent(filterField, 20)){
								...
								if(WebUI.waitForElementPresent(btnDeleteFilter, 20)){
									WebUI.click(btnDeleteFilter)

									if(WebUI.waitForElementNotPresent(btnDeleteFilter, 20)){

									}else{
										MesmerLogUtils.markFailed("Text is not deleted")
									}
								}else{
									MesmerLogUtils.markFailed("Unable to click on delete text")
								}
							}else{
