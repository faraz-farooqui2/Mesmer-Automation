...
def cancelRebootDevice(){
	if(WebUI.waitForElementPresent(deviceReboot, 20)){
		...
		if(WebUI.waitForElementPresent(confirmationRebootDialogue, 20)){
			...
			if(WebUI.waitForElementPresent(btnCancelReboot, 20)){
				WebUI.click(btnCancelReboot)
			}else{
				MesmerLogUtils.markFailed("Unable to click on cancel reboot button")
			}
		}else{
