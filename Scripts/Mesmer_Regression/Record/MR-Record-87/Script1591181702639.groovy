			...
			if(CreateTestController.getInstance().waitForRecordingStarts()){
				...
				if(WebUI.waitForElementPresent(btnSetting, 20)){
					...
					if(CreateTestController.getInstance().verifyOptionsAvailableOnSetting()){
						...
						if(WebUI.waitForElementPresent(btnWrapText, 20)){
							WebUI.click(btnWrapText)
							
								if(WebUI.waitForElementPresent(btnSetting, 20)){
									WebUI.click(btnSetting)
										
										if(WebUI.waitForElementPresent(btnUnwrapText, 20)){
											WebUI.click(btnUnwrapText)
										}else{
										MesmerLogUtils.markFailed("Unable to click on Unwrap Text icon")
										}
									
								}else{
									MesmerLogUtils.markFailed("Unable to click on setting icon")
								}
							
						}else{
							MesmerLogUtils.markFailed("Unable to click on WrapText icon")
						}
					}else{
