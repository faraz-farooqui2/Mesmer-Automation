...
//MR-Invite-04 | Empty email in invite users section
...
//			WebUI.click(AddInviteIcon)
			...
			if(WebUI.verifyElementPresent(Invite, 5)==true){
				//4. User clicks on 'Invite' button
				WebUI.click(Invite)

				if(WebUI.waitForElementVisible(txtInvalidEmailErrorMsg, 10)==true){
					KeywordUtil.markPassed("PASSED: Email address is empty")
					WebUI.delay(1)
					...
					KeywordUtil.markFailed("FAILED: No error for empty email address")
				}
