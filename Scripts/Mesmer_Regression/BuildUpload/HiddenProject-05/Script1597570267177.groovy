	...
	if(WebUI.waitForElementPresent(buildProjectPopOver, 60) ==true){
		...
		if(WebUI.waitForElementPresent(uploadNewBuildButton, 20)==true){
			...
			WebUI.delay(2)
				...
				String hiddenProj = "Hidden Projects ::"
							...
							List<WebElement> projList = driver.findElements(By.xpath(listProjectsXpath))
								...
								for (WebElement webElement : projList) {
									...
									if(projListText.contains(hiddenProjects)){
										MesmerLogUtils.logInfo("Hidden Project showing to Mesmer User " + ": " + hiddenProjects)
										MesmerLogUtils.markPassed("Mesmer user can see hidden project")
										break
									
									} else {
										MesmerLogUtils.logInfo(" Other projects showing to Mesmer User  " + " " + ":" + projListText)
