														...
														if(devicesList != null && devicesList.size() > 0){
															if(Utility.getPlatformName() == "Android") {
																if(WebUI.waitForElementPresent(androidDeviceList, 5) == true && WebUI.waitForElementPresent(iOSDeviceList, 5) == false ){
																	...
																	MesmerLogUtils.markFailed("iOS device found in Android Project")
																}
															}else if (Utility.getPlatformName() == "iOS") {
															if(WebUI.waitForElementPresent(androidDeviceList, 5) == false && WebUI.waitForElementPresent(iOSDeviceList, 5) == true ){
																MesmerLogUtils.markPassed("No Android device in iOS Project")
															}else{
																MesmerLogUtils.markFailed("Android device found in iOS Project")
															}
															}
														}
														else{
															MesmerLogUtils.markWarning("No devices available")
														}
