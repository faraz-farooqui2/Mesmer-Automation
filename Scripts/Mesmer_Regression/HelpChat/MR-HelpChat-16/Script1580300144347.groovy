		...
		if(WebUI.waitForElementPresent(btnSearchArticle, 30)){
			...
			if(searchResutsList != null && searchResutsList.size() == 3){
				if(WebUI.waitForElementPresent(btnSeeResults, 30)){
					...
					WebUI.delay(2)
					if(WebUI.waitForElementPresent(inputInterComSearchResults, 30)){
						WebUI.clearText(inputInterComSearchResults)
						WebUI.delay(2)
						WebUI.setText(inputInterComSearchResults, "build upload")
						WebUI.delay(2)
						WebUI.sendKeys(inputInterComSearchResults, Keys.chord(Keys.ENTER))
						WebUI.delay(5)
						String searchMoreResultsXpath = '//section[@class="section section__search_results"]/div'
						List<WebElement> searchMoreResutsList = driver.findElements(By.xpath(searchMoreResultsXpath))
						if(searchMoreResutsList != null && searchMoreResutsList.size() > 1){
							WebUI.closeWindowIndex(1)
							WebUI.delay(2)
							WebUI.switchToWindowIndex(0)
							WebUI.delay(2)
							KeywordUtil.markPassed("PASSED: MR-HelpChat-16 Successful")
						}
					}
					else{
						KeywordUtil.markFailed("FAILED: Input search more results not found")
					}
				}
				else{
