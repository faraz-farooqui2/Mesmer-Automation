	...
	if(WebUI.waitForElementPresent(spanSeeAll, 30)){
		...
		if(WebUI.waitForElementPresent(headerTitle, 30)){
			...
			if(conversationsList != null && conversationsList.size() > 0){
				conversationsList.get(0).click()
				WebUI.delay(3)
				if(WebUI.waitForElementPresent(inputWriteReplay, 30)){
					WebUI.click(inputWriteReplay)
					WebUI.setText(inputWriteReplay, "This is new message")
					WebUI.delay(2)
					if(WebUI.waitForElementPresent(btnSendMessage, 30)){
						...
						if(WebUI.waitForElementPresent(btnBack, 30)){
							WebUI.click(btnBack)
							WebUI.delay(3)
							KeywordUtil.markPassed("PASSED: MR-HelpChat-08 Successful")
						}
						else{
					...
					KeywordUtil.markFailed("FAILED: Input write replay not found")
				}
			}
		}
		else{
