...
private void waitForDeviceAndBuildInstallation(){
	if(WebUI.waitForElementVisible(divDevice, 240)){
		...
		rebootDevice()
		WebUI.delay(2)
		checkIfDeviceRebootedSuccessfully()
		WebUI.delay(5)
			...
			if(CreateTestController.getInstance().clickSettingsIcon()){
				if(CreateTestController.getInstance().checkIfAllSettingsOptionsAvailable()){
					if(CreateTestController.getInstance().clickSettingsOption("Download Log")){
						MesmerLogUtils.logInfo("MR-E2E-8 is successful")
						CreateTestController.getInstance().clickHideLogsIcon()
					}
				}
			}
		}
...
private void checkRedLogs(){
	List<WebElement> fontLogsList = CreateTestController.getInstance().getLogsByFontColor("red")
	if(fontLogsList != null && fontLogsList.size() > 0){
		MesmerLogUtils.logInfo("MR-E2E-9 is successful")
	}
	else{
		MesmerLogUtils.logInfo("No red log found in the logs list")
	}
}

private void checkYellowLogs(){
	List<WebElement> fontLogsList = CreateTestController.getInstance().getLogsByFontColor("yellow")
	if(fontLogsList != null && fontLogsList.size() > 0){
		MesmerLogUtils.logInfo("MR-E2E-10 is successful")
	}
	else{
		MesmerLogUtils.logInfo("No yellow log found in the logs list")
	}
}

private void rebootDevice(){
	if(CreateTestController.getInstance().rebootDevice()){
		MesmerLogUtils.logInfo("MR-E2E-11 is successful")
	}
}

private void checkIfDeviceRebootedSuccessfully(){
	if(WebUI.waitForElementVisible(divDevice, 300)){
		MesmerLogUtils.logInfo("MR-E2E-12 is successful")
	}
	else{
		MesmerLogUtils.logInfo("Device not rebooted with 5 minutes")
	}
}
