...
private void checkDevicesLogsOptions(){
	if(CreateTestController.getInstance().clickLogsIcon()){
		...
		if(CreateTestController.getInstance().checkIfGearIconAndShowFilterAvailable()){
			if(CreateTestController.getInstance().clickShowFilter()){
				if(CreateTestController.getInstance().checkIfShowFilterContainsAllOptions()){
					if(CreateTestController.getInstance().clickShowFilterOption("Debug")){
						MesmerLogUtils.logInfo("MR-E2E-7 is successful")
					}
				}
			}
		}
