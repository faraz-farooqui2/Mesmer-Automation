...
/*
 * MR-Manage Test Case-07 | Verify single test case title name and it should be editable														
 */
...
try{
	if(checkTestCaseAndPerformActions()){
		MesmerLogUtils.markPassed("MS-ManageTest-07 Successful")
	}
}
catch(Exception e){
	e.printStackTrace()
...
private boolean checkTestCaseAndPerformActions(){
	...
	if(testCasesList != null && testCasesList.size() > 0){
		if(ManageTestController.getInstance().findTestCaseAndPerformAction("ManageTest-Automation", "edit")){
			if(ManageTestController.getInstance().clickEditTestIcon()){
				if(ManageTestController.getInstance().editTestCaseTitle("ManageTest-Automation-Updated")){
					result = true
				}
