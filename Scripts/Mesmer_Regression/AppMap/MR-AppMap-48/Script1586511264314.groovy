...
//AppMap-48 | Verify simulators for crawl [IOS build with .app extension]
				...
				if(AppMapController.getInstance().clickNextButton()){
					...
					if(AppMapController.getInstance().verifyDeviceDialogue()){
						...
						if(virtualDeviceList.size() > 0 ){
							...
							KeywordUtil.logInfo("Virtual Devices available :" + " " + virtualDeviceList.size())

						}
						else{
							KeywordUtil.markWarning("No virtual devices found")
						}
					}else{
