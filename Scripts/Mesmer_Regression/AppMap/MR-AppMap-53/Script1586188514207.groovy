...
try{
	...
	if(WebUI.waitForElementVisible(btnHistory, 5)==true){
		WebUI.mouseOver(btnHistory)
		MesmerLogUtils.logInfo("History Icon is displayed")
		
		if(WebUI.waitForElementVisible(tooltipHistory, 5)==true){
			MesmerLogUtils.markPassed("AppMap TC#53 is Passed \n History Tooltip is displayed")
		}
		else{
			MesmerLogUtils.markFailed("AppMap TC#53 is Failed \n History Tooltip is not displayed")
		}
