...
//AppMap-45 | Verify emulators for crawl [Android x86]
			...
			if(AppMapController.getInstance().verifyConfigureCrawlDialogue()){
				if(AppMapController.getInstance().clickNextButton()){
					...
					if(AppMapController.getInstance().verifyDeviceDialogue()){
						...
						if(physicalDeviceList.size() > 0 || virtualDeviceList.size() > 0){

							KeywordUtil.markPassed("Physical : " + physicalDeviceList.size() + " & " + "Virtual :" + virtualDeviceList.size() +" " + "devices are compatible with this build")
						}
						else{
							KeywordUtil.markWarning("Physical/Virtual device is not compatible with the build")
						}
					}else{
