...
// MR-Device Selection-51 | Rerun | Verify user should be able to see a Broken device
	...
	if(WebUI.waitForElementPresent(btnNew, 20)== true){
		...
		if(titleStreamList.size() > 0){
			...
			if(WebUI.waitForElementPresent(btnReplay , 120)==true){
				...
				if(WebUI.waitForElementPresent(listBrokenDevices, 10)==true){
					MesmerLogUtils.markFailed("Broken device found")
