...
// MR-Device Selection-54 | Rerun | Verify user should be able to see if a device is in Use by any other user
	...
	if(WebUI.waitForElementPresent(btnNew, 20)== true){
		...
		if(titleStreamList.size() > 0){
			...
			if(WebUI.waitForElementPresent(btnReplay , 120)==true){
				...
				if(WebUI.waitForElementPresent(listInUseDevices, 10)==true){
					MesmerLogUtils.markPassed("InUse device found")

				}else{
					MesmerLogUtils.logInfo("No InUse device found")
				}
			}else{
