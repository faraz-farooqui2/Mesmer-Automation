...
// MR-DeviceQueue-14 | Verify user should be able to stop/delete/clear the in-queue task by hover over the task in 'Queue' section and clicking the cross (x) icon
...
try{
	...
	if(DeviceManagerController.getInstance().clickOnEyeIcon()){
		if(DeviceManagerController.getInstance().verifyDeviceQueueDialogue()){
			if(DeviceManagerController.getInstance().removeSingleTestFromDeviceQueue()){
				WebUI.delay(2)
				...
				MesmerLogUtils.markFailed("removeSingleTestFromDeviceQueue method failed")
			}
		}else{
