...
//MR-Device Selection-82 | Record | Verify user should be able to see if a device is Broken
			...
			if(AppMapController.getInstance().verifyConfigureCrawlDialogue()){
				if(AppMapController.getInstance().clickNextButton()){
					...
					if(AppMapController.getInstance().verifyDeviceDialogue()){
						...
						String ListBrokenDevices = findTestObject('Object Repository/OR_AppMap/list_BrokenDevice').findPropertyValue('xpath').toString()
						List<WebElement> numberOfUBrokenDevices = driver.findElements(By.xpath(ListBrokenDevices))

						if(numberOfDevices != null && numberOfDevices.size() >0){

							MesmerLogUtils.logInfo("Number of broken devices in Crawl Devices list = " + numberOfUBrokenDevices.size())

						}

						else{
							MesmerLogUtils.logInfo("There is no broken available")
						}
