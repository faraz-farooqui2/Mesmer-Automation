...
try{
	if(checkTestCaseAndPerformActions()){
		MesmerLogUtils.markPassed("MR-RecommendedTest-25 successful")
	}
}
catch(Exception e){
	e.printStackTrace()
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		...
		if(testCasesList != null && testCasesList.size() > 0){
			if(RecommendedTestCaseController.getInstance().clickTestCaseTitle("Recommended Test 1")()){
				String testCasename = 'Recommended Test 1 Updated'
				if(RecommendedTestCaseController.getInstance().editTitleInInputFieldWithoutEnter(testCasename)){
					if(RecommendedTestCaseController.getInstance().clickEditCloseBtn()){
						String name = RecommendedTestCaseController.getInstance().getTestCaseTitle(0)
						if(name.equalsIgnoreCase("Recommended Test 1")){
							result = true
						}
					}
				}
