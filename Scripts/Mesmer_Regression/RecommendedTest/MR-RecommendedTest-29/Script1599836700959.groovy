...
try{
	if(checkTestCaseAndPerformActions()){
		MesmerLogUtils.markPassed("MR-RecommendedTest-29 successful")
	}
}
catch(Exception e){
	e.printStackTrace()
...
private boolean checkTestCaseAndPerformActions(){
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		...
		if(testCasesList != null && testCasesList.size() > 0){
			if(RecommendedTestCaseController.getInstance().findElementAndPerformAction("Recommended Test 1","acceptAllTests")){
				result = true
			}
		}
	}
