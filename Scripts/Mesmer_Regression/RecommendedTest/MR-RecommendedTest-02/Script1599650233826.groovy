...
try{
	checkTestCaseAndPerformActions()
}
catch(Exception e){
	e.printStackTrace()
}
...
private boolean checkTestCaseAndPerformActions(){
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		if(RecommendedTestCaseController.getInstance().checkIfDismissAndProvideTestDataExists()){
			if(TestDataController.getInstance().checkIfProvideTestDataScreenAppears()){
				result = true
				MesmerLogUtils.markPassed("MR-RecommendedTest-02 successful")
			}
		}
	}
	return result
