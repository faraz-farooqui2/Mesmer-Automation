...
try{
	if(checkTestCaseAndPerformActions()){
		MesmerLogUtils.markPassed("MR-RecommendedTest-10 successful")
	}
}
catch(Exception e){
	e.printStackTrace()
...
private boolean checkTestCaseAndPerformActions(){
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		if(RecommendedTestCaseController.getInstance().selectShowScreensFilterOption("Untitled Screens")){
			if(RecommendedTestCaseController.getInstance().clickScreensFilterClearAllBtn()){
				result = true
			}
		}
	}
