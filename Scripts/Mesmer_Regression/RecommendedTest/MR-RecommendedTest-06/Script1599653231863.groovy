...
private boolean checkTestCaseAndPerformActions(){
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		...
		if(testCasesList != null && testCasesList.size() > 0){
			if(RecommendedTestCaseController.getInstance().checkIfTestCasesExpanded(0)){
				if(RecommendedTestCaseController.getInstance().collapseTestCasesTab(0)){
					result = true
					MesmerLogUtils.markPassed("MR-RecommendedTest-06 successful")
				}
			}
		}
	}
