...
try{
	if(checkTestCaseAndPerformActions()){
		MesmerLogUtils.markPassed("MR-RecommendedTest-11 successful")
	}
}
catch(Exception e){
	e.printStackTrace()
...
private boolean checkTestCaseAndPerformActions(){
	...
	if(RecommendedTestCaseController.getInstance().checkIfRecommendedTestScreenOpen()){
		if(RecommendedTestCaseController.getInstance().selectShowSortByFilterOption("Screen Name")){
			if(RecommendedTestCaseController.getInstance().selectShowSortByFilterOption("Status")){
				result = true
			}
		}
	}
